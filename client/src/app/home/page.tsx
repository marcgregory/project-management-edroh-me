...
const HomePage = () => {
  ...
  const priorityCount = tasks.reduce(
    ...
    // acc = accumulator object that stores counts
    // task = current task in the array
    (acc: Record<string, number>, task: Task) => {
      // Extract the priority value from the current task
      const { priority } = task;
      // Increase the count for this priority
      // If acc[priority] does not exist, default to 0, then add 1
